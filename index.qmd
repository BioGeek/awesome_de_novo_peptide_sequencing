...
<script>
// Tap-to-toggle the filter pane on mobile widths (≤768px).
// On wider viewports, leave the panel always expanded — the script only
// flips aria-expanded so the chevron and body visibility CSS reacts.
(() => {
  ...
  // Only interactive on mobile widths; on desktop the CSS shows the body unconditionally.
  const wireUp = () => {
...
Snapshots of this catalog are archived on Zenodo with a citable DOI. The concept DOI below resolves to the latest version; individual versions get their own DOI under the same concept.
...
Machine-readable metadata: [`CITATION.cff`](https://github.com/BioGeek/awesome_de_novo_peptide_sequencing/blob/main/CITATION.cff). GitHub's "Cite this repository" button exports BibTeX / APA / RIS / EndNote.
