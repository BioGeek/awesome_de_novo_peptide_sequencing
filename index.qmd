...
Authors and the organizations behind them span **${n_countries}** countries. Circle area scales with the selected impact metric; colour groups organizations by type. Nearby organizations are gently separated so dense hubs remain readable without zooming. Organization type is inferred from institution names and should be read as a practical display category.
...
where_the_work_map = {
  ...
  const max_value = d3.max(org_data, d => d[org_metric]) || 1
  const radius = v => 3 + 24 * Math.sqrt((v || 0) / max_value)

  const nodes = org_data.map(d => {
    const [x0, y0] = projection([d.lng, d.lat])
    return { ...d, x0, y0, x: x0, y: y0, r: radius(d[org_metric]) }
  })
  // Collision layout nudges dense hubs apart while keeping circles pulled toward
  // their true projected coordinates.
  const sim = d3.forceSimulation(nodes)
    .force("x", d3.forceX(d => d.x0).strength(0.35))
    .force("y", d3.forceY(d => d.y0).strength(0.35))
    .force("collide", d3.forceCollide(d => d.r + 1.6).iterations(3))
    .stop()
  ...
  const zoom = d3.zoom()
    ...
    .on("zoom", ev => map_layer.attr("transform", ev.transform))
  ...
  // Leader lines reveal how far collision has moved circles from their true
  // geocoded anchor.
  map_layer.append("g")
    .attr("stroke", "#8c959f")
    ...
    .selectAll("line")
    .data(nodes.filter(d => Math.hypot(d.x - d.x0, d.y - d.y0) > 8))
    .join("line")
    .attr("x1", d => d.x0)
    .attr("y1", d => d.y0)
    .attr("x2", d => d.x)
  ...
  const org_layer = map_layer.append("g")
  org_layer.selectAll("circle")
    .data(nodes)
    .join("circle")
    .attr("cx", d => d.x)
    .attr("cy", d => d.y)
    .attr("r", d => d.r)
    .attr("fill", d => org_type_color[d.type])
    .attr("fill-opacity", 0.78)
    .attr("stroke", "#1f2328")
    .attr("stroke-width", 0.55)
    .append("title").text(d => `${d.organization}
${d.type} · ${d.city}, ${d.country}
${d.papers} papers · ${d.authors} authors · ${d.citations} global citations
${d.affiliations} affiliation rows`)

  // Emphasize the most impactful organizations with short labels.
  const labelled = nodes
    .slice()
    .sort((a, b) => d3.descending(a[org_metric], b[org_metric]))
    .slice(0, 18)
  map_layer.append("g")
    .selectAll("text")
    .data(labelled)
    ...
    .attr("stroke", "white")
    .attr("stroke-width", 3)
    .text(d => d.organization.length > 26 ? d.organization.slice(0, 24) + "…" : d.organization)
  ...
  legend.append("text").attr("x", 0).attr("y", -10)
    ...
    .text(`${nodes.length} organizations · circle area ∝ ${metric_label}`)
  ...
  size_legend.append("circle").attr("cx", 28).attr("cy", 36 + max_r - radius(max_value / 4)).attr("r", radius(max_value / 4)).attr("fill", "none").attr("stroke", "#57606a")
...
Publication-level global citation counts from **OpenAlex** `cited_by_count`. Counts are matched to catalog publications by DOI first, then by high-confidence title search when DOI lookup is unavailable; refresh with `build_publication_impact.py`.
...
Open-source uptake snapshot from the GitHub API: stars, fork count, open / closed issues + PRs, and the most recent push. Pulled offline by `build_repo_metrics.py` against `algorithm_repository.url`. Non-GitHub repos (PyPI, project home pages, anonymised review repos) aren't counted here.
...
//| echo: false
{
  ...
  const table = Inputs.table(rows, {
    columns: ["model", "family", "stars", "forks", "open_issues",
              "closed_issues", "open_prs", "closed_prs", "last_pushed", "url"],
    header: {
      ...
      last_pushed: "Last updated", url: "Repo"
    },
    format: {
