...
innovations_timeline = {
  ...
  const chart = Plot.plot({
    ...
    marks: [
      ...
      // Dots are drawn via a Plot.dot placeholder so the layout is computed
      // (x/y scales, viewBox etc.), then replaced with d3-controlled circles
      // below so we can wire our .map-tooltip HTML style. Empty title means
      // Plot won't add native <title> tooltips.
      Plot.dot(items, {
        x: "first_pub",
        y: "y",
        fill: "family",
...
network_chart = {
  ...
  node_g.attr("aria-label", d =>
    `${d.id}. ${d.affiliations.join(", ")}. ${d.degree} co-author links.`
  )
  const tooltip_el = html`<div class="map-tooltip" role="tooltip" aria-hidden="true"></div>`
  const move_tip = ev => {
    const pad = 12
  ...
  const show_tip = (ev, d) => {
    tooltip_el.replaceChildren(html`<div>
      ...
      <div class="map-tooltip-meta">${d.degree} co-author link${d.degree === 1 ? "" : "s"}</div>
