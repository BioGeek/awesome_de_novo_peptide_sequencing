...
citation_arcs = {
  ...
  const cite_count = new Map()  // pub_id → in-degree (times cited)
  for (const e of citations_t) cite_count.set(e.cited_id, (cite_count.get(e.cited_id) ?? 0) + 1)

  // Y-strata by kind. Same order as the breakdown panel; benchmarks/reviews/meta share a row.
  const KIND_ORDER = [
