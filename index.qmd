...
Which sequencing tools does each application community actually use? Inferred from the intra-catalog citation graph: an edge from *palaeoproteomics* to *Casanovo* means at least one palaeoproteomics-tagged paper in the catalog cites the canonical Casanovo publication. Edge thickness is the count. Some subdomains cite classical / non-DL tools (PEAKS, PepNovo) — those show up on the right side too. Palaeoproteomics currently has no visible flows because its papers pre-date most modern DL sequencers and mostly cite PEAKS-family tools not tracked as citation targets.
