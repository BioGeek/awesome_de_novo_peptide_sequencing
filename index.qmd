...
innovations_timeline = {
  ...
  const band_heights = {
    ...
    "CNN + RNN":         3.5,   // 2018-2024 cluster of DeepNovo variants + neighbours
    "Transformer (AR)":  8.0,
    "GNN":               1.2,
    "CNN":               1.2,
    "Transformer (NAR)": 2.5,
...
Which sequencing tools does each application community actually use? Read straight from the curated tool-usage links: an edge from *metaproteomics* to *PEAKS* means at least one metaproteomics-tagged paper reports using PEAKS. Edge thickness is the paper count. Both DL and classical sequencers show up on the right side, alongside adjacent database-search and rescoring tools.
...
application_sankey = {
  ...
  // report using that sequencer. Uses each publication's own curated
  // publication_algorithm links (via pubs_t.models), NOT the Crossref
  // reference graph. This surfaces every subdomain the moment a paper is
  // added, rather than waiting for the monthly build_citations.py cron.
  const flow = new Map()
  ...
  if (flow.size === 0) {
    return html`<p style="color:#57606a; font-style:italic; padding:1rem;">
      ...
      have no <code>publication_algorithm</code> links to indexed sequencing methods.
    </p>`
  }
