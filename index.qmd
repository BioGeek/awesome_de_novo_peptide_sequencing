...
innovations_timeline = {
  ...
  const arch_dot_circles = chart.querySelectorAll(".arch-dot circle")
...
application_timeline = {
  ...
  const app_dot_circles = chart.querySelectorAll(".app-dot circle")
...
//| echo: false
{
  ...
  chart.querySelectorAll(".repo-bar rect").forEach((rect, i) => {
    const d = sorted[i]
    if (!d) return
  ...
  return html`<div class="chart-wrap">${chart}${repo_bar_tooltip}</div>`
}
```
...
//| echo: false
{
  ...
  chart.querySelectorAll(".repo-dot circle").forEach((circle, i) => {
    const d = points[i]
    if (!d) return
    circle.style.cursor = "pointer"
    ...
    circle.addEventListener("mouseenter", ev => {
      repo_dot_tooltip.replaceChildren(html`<div>
        ...
        ${d.family ? html`<div class="map-tooltip-row">
          <span class="map-tooltip-swatch" style=${`background:${d.family_color ?? "#4C72B0"}`}></span>
          <span>${d.family}</span>
        </div>` : ""}
  ...
  return html`<div class="chart-wrap">${chart}${repo_dot_tooltip}</div>`
}
```
