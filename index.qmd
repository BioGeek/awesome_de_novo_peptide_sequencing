...
innovations_timeline = {
  ...
  // Dynamic MIN_GAP_DAYS: how many days of horizontal space a "wide-ish" label needs.
  // Chart canvas is 1400 px wide; with marginLeft=140 and marginRight=60 the plot area
  // is ~1200 px. LABEL_RESERVE_PX is the worst-case label width we want to clear.
  const PLOT_WIDTH_PX  = 1200
  const LABEL_RESERVE_PX = 110   // generous for names like "DeepNovo Peptidome", "Charge Derivatization"
  const total_days = (x_pad_max - x_pad_min) / 86400000
  const px_per_day = PLOT_WIDTH_PX / total_days
  const MIN_GAP_DAYS = LABEL_RESERVE_PX / px_per_day
