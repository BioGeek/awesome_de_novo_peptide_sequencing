...
//| echo: false
lifecycle_rows = {
  // Group publications by their linked method name (using `models` from pubs_t).
  // A pub without a model link is bucketed under a per-pub synthetic key so it
  // doesn't accidentally pair with another stand-alone pub.
  const by_method = new Map()
  for (const p of pubs_filtered) {
    const key = (p.models ?? "").split(",")[0]?.trim() || `_pub_${p.id}`
    if (!by_method.has(key)) by_method.set(key, [])
    by_method.get(key).push(p)
  }

  const POST_PREPRINT = new Set(["peer-reviewed", "ML conference", "thesis"])

  const rows = []
  for (const [method, pubs] of by_method.entries()) {
...
//| echo: false
{
  const table = Inputs.table(lifecycle_search, {
    columns: ["method", "status", "preprint_date", "peer_date", "gap_months"],
    header: {
      method:        "Method",
      status:        "Status",
    ...
    rows: 25,
    width: { method: 220, status: 160, preprint_date: 110, peer_date: 110, gap_months: 110 }
  })
  return html`<div class="chart-wrap">
    <button class="fs-btn" onclick="
      const el = this.parentElement;
...
//| echo: false
{
  // Some methods (Casanovo, Lutefisk) have multiple paired publications; if we
  // y='method' the bars stack visually and the apparent gap becomes the sum.
  // Give each pairing a unique y label so every row renders as its own bar.
  const method_counts = new Map()
  for (const r of lifecycle_rows.filter(r => r.status === "paired")) {
    method_counts.set(r.method, (method_counts.get(r.method) ?? 0) + 1)
  }
  // Use a "year-aware" label whenever the same method appears more than once.
  // Pull the year robustly: preprint_date may arrive as Date or as a string,
  // depending on how Quarto serialized the row through the data chunk.
  const yearOf = d => {
    if (!d) return ""
  ...
  const paired = lifecycle_rows
    ...
    .map(r => {
      const dup = (method_counts.get(r.method) ?? 0) > 1
      const yr  = yearOf(r.preprint_date)
      return { ...r, label: dup && yr ? `${r.method} (${yr})` : r.method }
    })
  if (!paired.length) {
...
//| echo: false
{
  ...
  const table = Inputs.table(search_with_if, {
    columns: ["year", "models", "kind", "is_dl", "acquisition", "title", "authors", "journal", "citedness", "type", "repo"],
    header: {
    ...
    reverse: true,
    rows: 25,
    width: { year: 60, type: 100, models: 130, kind: 130, is_dl: 70, acquisition: 70, citedness: 70, repo: 60 }
  })
  return html`<div class="chart-wrap">
    <button class="fs-btn" onclick="
      const el = this.parentElement;
