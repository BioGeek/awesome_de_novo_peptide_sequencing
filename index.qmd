...
Which sequencing tools does each application community actually use? Two signals are unioned: (1) explicit `publication_algorithm` links — the curator's "this paper uses tool X" assertion; (2) the Crossref citation graph — "this paper cites a publication describing tool X" (rebuilt monthly by `build_citations.py`). An edge from *metaproteomics* to *PEAKS* means at least one metaproteomics-tagged paper either uses or cites PEAKS. Edge thickness is the paper count. Both DL and classical sequencers show up on the right side, alongside adjacent database-search and rescoring tools.
...
application_sankey = {
  ...
  if (flow.size === 0) {
    return html`<p style="color:#57606a; font-style:italic; padding:1rem;">
      ...
      have neither <code>publication_algorithm</code> links nor citations to
      indexed sequencing methods.
    </p>`
  }
