...
application_timeline = {
  ...
  // MIN_GAP_DAYS auto-tunes to the actual x-scale, capped so dense years
  // (2018/2020/2022 in venomics) don't all fall through to the fallback tier.
  const PLOT_WIDTH_PX = 950            // chart 1200 minus 210 left + 40 right margins
  const LABEL_RESERVE_PX = 110         // median label width after the 7b6440a rename
  const MAX_GAP_DAYS = 550             // cap ~1.5 yr
  const total_days = (x_pad_max - x_pad_min) / 86400000
  const px_per_day = PLOT_WIDTH_PX / total_days
  const MIN_GAP_DAYS = Math.min(MAX_GAP_DAYS, LABEL_RESERVE_PX / px_per_day)
  ...
  const chart = Plot.plot({
    ...
    marks: [
      ...
      // Labels rotated -30° so long taxon-focused names ("Anatolian meadow viper
      // top-down ISD venomics" = 44 chars ≈ 350 px unrotated) stack diagonally
      // instead of running horizontally into each other. Standard timeline pattern.
      // Upper half of lane → label goes up-right; lower half → label goes down-right.
      Plot.text(items.filter(d => d.y_frac >= 0.5), {
      ...
      Plot.text(items.filter(d => d.y_frac < 0.5), {
        x: "first_pub", y: "y", text: "model",
        textAnchor: "start", lineAnchor: "top",
        dx: 8, dy: 4, rotate: 30,
        fontSize: 10, fontWeight: "bold", fill: "subdomain"
      })
    ]
