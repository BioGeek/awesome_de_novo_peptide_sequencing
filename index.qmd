...
innovations_timeline = {
  ...
  // band_order names only the thirteen busiest families; the checkbox list offers all
  // of them (49 at the time of writing). Intersecting with band_order alone silently
  // dropped the other 36, so checking one of those produced an empty chart. Append the
  // unregistered families after the canonical order instead, the same way the
  // Application-areas lanes handle an unregistered subdomain.
  const extra_bands = Array.from(present).filter(f => !band_order.includes(f)).sort(d3.ascending)
  const visible_bands = band_order.filter(f => present.has(f)).concat(extra_bands)
...
only view here that shows all of them: the chart above groups methods into
thirteen architectural bands and the volume chart below counts papers per year,
where the early era is a one-paper sliver against a linear axis.`
```
