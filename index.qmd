  ...
  node
    .on("mouseenter", (ev, d) => {
      arc_tooltip.replaceChildren(html`<div>
        <div class="map-tooltip-title">${d.models || "(no model)"}</div>
        <div class="map-tooltip-meta">${d.title}</div>
        <div class="map-tooltip-row">
          <span class="map-tooltip-swatch" style=${`background:${kind_color[d.kind] ?? "#999"}`}></span>
          <span>${d.kind}${d.year ? ` · ${d.year}` : ""}</span>
        </div>
        <div class="map-tooltip-row"><span>cited by <b>${d.in_deg}</b> · cites <b>${d.out_deg}</b></span></div>
      </div>`)
      arc_tooltip.classList.add("visible")
      arc_tooltip.setAttribute("aria-hidden", "false")
      move_arc_tip(ev)
    })
    .on("mousemove", move_arc_tip)
    .on("mouseleave", () => {
