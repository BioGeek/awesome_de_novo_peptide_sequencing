...
Plot popularity (★, log scale) against staleness (months since last push). Vibrant repos cluster on the right at higher star counts; abandoned-but-historically-popular projects drift toward the left. Dot size scales with open issues + open PRs.
...
//| echo: false
{
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      // dodgeY auto-stacks labels that would collide at the same (x, y) so
      // sibling repos with similar stats (e.g. InstaNovo + InstaNovo-P,
      // PrimeNovo + RefineNovo) no longer print on top of each other.
      Plot.text(points.filter(d => d.stars >= 20),
        Plot.dodgeY({anchor: "middle", padding: 2}, {
          x: "months_since_push", y: "stars", text: "model",
