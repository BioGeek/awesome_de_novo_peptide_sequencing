...
coauth_edges = pd.read_sql(
    """
    SELECT a1.name   AS source,
           a2.name   AS target,
           COUNT(*)  AS weight,
           ac1.pubs  AS source_pubs,
           ac2.pubs  AS target_pubs
    ...
    JOIN (SELECT author_id, COUNT(*) AS pubs FROM publication_author GROUP BY author_id) ac1 ON ac1.author_id = pa1.author_id
    JOIN (SELECT author_id, COUNT(*) AS pubs FROM publication_author GROUP BY author_id) ac2 ON ac2.author_id = pa2.author_id
    WHERE pa1.author_id < pa2.author_id
      AND ac1.pubs >= 2 AND ac2.pubs >= 2
...
//| echo: false
{
  // Per-country distinct authors / affiliations, recomputed from the currently
  // filtered pubs so the chart reacts to the global Kind / Approach /
  // Acquisition filters. Falls back to the unfiltered geo_t totals when no
  // filter has been applied (avoids an empty chart on first paint).
  const pub_ids = new Set(pubs_filtered.map(p => p.id))
  const authors_by_country = new Map()
  const affs_by_country    = new Map()
  for (const r of pub_authorship_t) {
    if (!pub_ids.has(r.pub_id) || !r.country) continue
    if (!authors_by_country.has(r.country)) authors_by_country.set(r.country, new Set())
    if (!affs_by_country.has(r.country))    affs_by_country.set(r.country,    new Set())
    authors_by_country.get(r.country).add(r.author_id)
    affs_by_country.get(r.country).add(r.affiliation_id)
  }
  const rows = Array.from(authors_by_country.keys()).map(country => ({
    country,
    authors:      authors_by_country.get(country).size,
    affiliations: affs_by_country.get(country).size
  })).filter(r => r[country_metric] > 0)

  return Plot.plot({
    marginLeft: 140,
    width: 1100,
    height: Math.max(280, rows.length * 22),
    x: { label: country_metric === "authors" ? `Distinct authors (${pubs_filtered.length} papers)` : `Distinct affiliations (${pubs_filtered.length} papers)`, grid: true },
    y: { label: null },
    marks: [
      Plot.barX(rows, {
        x: country_metric,
...
The chart shows the twenty most-published authors. The network below it shows how prolific authors are connected through co-authorship — pick a publication threshold to expand or tighten the graph. Drag a node to reshape the layout, or hover to highlight a neighborhood.

```{ojs}
//| echo: false
viewof coauth_threshold = Inputs.radio(
  [2, 3],
  { label: "Show authors with ≥ N papers", value: 3 }
)
```
...
network_chart = {
  ...
  // SQL ships every coauth edge where both authors have ≥ 2 papers; here we
  // further filter to the user's chosen threshold (≥ 2 or ≥ 3) so the graph
  // can be loosened to see weaker collaborations.
  const edges = coauth_edges_t.filter(
    e => (e.source_pubs ?? 0) >= coauth_threshold && (e.target_pubs ?? 0) >= coauth_threshold
  )
  const node_set = new Set()
  for (const e of edges) { node_set.add(e.source); node_set.add(e.target) }
  ...
  svg.append("text")
    ...
    .attr("fill", "#24292f")
    .text(`Co-authorship network — authors with ≥ ${coauth_threshold} papers (${nodes.length} authors, ${links.length} edges), pie nodes for multi-affiliation`)
...
author_algo_network = {
  ...
  // Prolific authors (≥ 3 papers) — kept fixed here independent of the co-auth
  // threshold radio above, because the bipartite chart loses meaning if every
  // single-paper collaborator gets a model node attached.
  const prolific = new Set(
    coauth_edges_t
      .filter(e => (e.source_pubs ?? 0) >= 3 && (e.target_pubs ?? 0) >= 3)
...
Edges resolved from **Crossref** (by DOI) and **Semantic Scholar** (by DOI or title-search fallback), matched back to publications via DOI-exact (and, for refs without a DOI, fuzzy-title with token-set ratio ≥ 92). Only intra-catalog citations are drawn — references to papers outside the catalog are filtered out.
