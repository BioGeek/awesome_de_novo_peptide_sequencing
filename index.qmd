...
//| echo: false
{
  ...
  // Disambiguate the y-label per pairing so every pairing gets its own bar.
  ...
  const paired = lifecycle_rows
    ...
    .map(r => ({
      ...r,
      // Tag pairings with the preprint year only when the method has more than one.
      label: method_counts.get(r.method) > 1 && r.preprint_date
        ? `${r.method} (${r.preprint_date.getFullYear()})`
        : r.method
    }))
  if (!paired.length) {
