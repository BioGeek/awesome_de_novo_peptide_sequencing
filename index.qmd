...
A directed citation graph between papers in this catalog. Edges were resolved from **Crossref** (by DOI) and **Semantic Scholar** (by DOI or title-search fallback), then matched back to publications via DOI-exact and fuzzy-title (token-set ratio ≥ 92). Only intra-catalog citations show up — references to papers outside the catalog are filtered out. Arrows point from the citing paper to the cited paper.
...
citation_graph = {
  ...
  // Per-publication metadata for node tooltips and coloring.
  const pub_by_id = new Map(pubs_t.map(p => [p.id, p]))

  // Build the node set from the edge endpoints (isolated pubs are dropped).
  const node_ids = new Set()
  for (const e of citations_t) { node_ids.add(e.citing_id); node_ids.add(e.cited_id) }
  const nodes = Array.from(node_ids, id => {
  ...
  const links = citations_t.map(e => ({
    source: e.citing_id, target: e.cited_id, source_kind: e.source
  }))
  for (const l of links) {
    node_by_id.get(l.source).out_deg += 1
    node_by_id.get(l.target).in_deg  += 1
  }

  // Coloring by kind — same palette as the breakdown panel.
  const kind_color = {
    "algorithm":              "#1f6feb",
  ...
  const nodeR = d => 4 + 12 * Math.sqrt(d.in_deg / max_in)
  ...
  svg.append("text")
    ...
    .attr("font-weight", "bold")
    .attr("fill", "#24292f")
    .text(`Citation graph — ${nodes.length} papers, ${links.length} intra-catalog edges`)
  ...
  const link = g.append("g")
    ...
    .data(links)
    .join("line")
    .attr("stroke", d => d.source_kind === "both" ? "#374151" : "#999")
    .attr("stroke-width", d => d.source_kind === "both" ? 1.4 : 0.9)
    .attr("marker-end", "url(#cite-arrow)")
  ...
  node.append("title").text(d =>
    `${d.models || "(no model)"} — ${d.title}\nyear: ${d.year}\nkind: ${d.kind}\n${d.in_deg} cited by · ${d.out_deg} cites`
  )

  // Label only the most-cited nodes to keep the canvas readable.
  const label_threshold = Math.max(2, d3.quantile(nodes.map(n => n.in_deg).sort((a, b) => a - b), 0.85) || 2)
  const label = g.append("g")
    .attr("pointer-events", "none")
    .selectAll("text")
    .data(nodes.filter(n => n.in_deg >= label_threshold))
    .join("text")
    .text(d => d.models?.split(",")[0]?.trim() || d.title.slice(0, 28))
    .attr("font-size", 10)
  ...
  const legend_kinds = Array.from(new Set(nodes.map(n => n.kind))).sort()
  const legend_g = svg.append("g").attr("transform", `translate(40, ${height - 25})`)
  legend_g.selectAll("g.leg")
    .data(legend_kinds)
    .join("g")
    .attr("class", "leg")
    .attr("transform", (_, i) => `translate(${i * 170}, 0)`)
    .call(s => {
      s.append("circle").attr("r", 6).attr("cx", 6).attr("cy", -4).attr("fill", k => kind_color[k] ?? "#999")
      s.append("text")
        .attr("x", 18).attr("y", 0)
        .attr("font-size", 11)
        .attr("fill", "#24292f")
...
//| echo: false
{
  const table = Inputs.table(search, {
    columns: ["year", "models", "kind", "is_dl", "acquisition", "title", "authors", "journal", "type", "repo"],
    header: {
      year: "Year", models: "Method(s)", kind: "Kind", is_dl: "DL?", acquisition: "Acq.",
      title: "Title", authors: "Authors", journal: "Venue", type: "Type", repo: "Code"
    },
    format: {
    ...
    reverse: true,
    rows: 25,
    width: { year: 60, type: 100, models: 130, kind: 130, is_dl: 70, acquisition: 70, repo: 60 }
  })
  return html`<div class="chart-wrap">
    <button class="fs-btn" onclick="
      const el = this.parentElement;
