...
*De novo* peptide sequencing gets picked up by a handful of distinct scientific communities, each with its own workflow conventions. Every algorithm classified `kind='downstream-application'` carries a `subdomain` tag; each dot below is one application-focused method / workflow, placed at its first publication date and stacked into a lane by sub-domain.
...
application_timeline = {
  ...
  const lane_order = [
    ...
    "immunopeptidomics", "antibodyomics", "palaeoproteomics"
  ]
  const lane_label = {
  ...
  const Y_TIERS = [0.5, 0.85, 0.15, 0.7, 0.3, 0.9, 0.1, 0.6, 0.4]
  ...
  const MIN_GAP_DAYS = 400  // wider than the architectures chart — labels here can be long ('Streptolysin O epitope de novo workflow')
  ...
  const chart = Plot.plot({
    ...
    marks: [
      ...
      Plot.text(items.filter(d => d.y_frac >= 0.5), {
        x: "first_pub", y: "y", text: "model",
        textAnchor: "middle", lineAnchor: "bottom", dy: -10,
        fontSize: 10, fontWeight: "bold", fill: "subdomain"
      }),
      Plot.text(items.filter(d => d.y_frac < 0.5), {
        x: "first_pub", y: "y", text: "model",
        textAnchor: "middle", lineAnchor: "top", dy: 10,
        fontSize: 10, fontWeight: "bold", fill: "subdomain"
      })
    ]
  })
...
Which sequencing tools does each application community actually use? Inferred from the intra-catalog citation graph: an edge from *palaeoproteomics* to *Casanovo* means at least one palaeoproteomics-tagged paper in the catalog cites the canonical Casanovo publication. Edge thickness is the paper count. Both DL and classical sequencers (PEAKS, PepNovo, Novor, pNovo, ...) show up on the right side, since every `kind='algorithm'` row is a valid target.

Palaeoproteomics flows look thin here for two reasons: (1) Crossref's reference-list coverage for older Nature papers is patchy, so a typical palaeo paper produces only 0–4 outgoing edges in `publication_citation` versus 20–30 for a modern DL paper; (2) the palaeo bibliography's centre of gravity is palaeogenetics / anthropology / palaeontology — off-catalog and therefore invisible here. What survives (`palaeoproteomics → PEAKS`, from the ostrich-eggshell paper) is genuine but under-represents actual tool use.
...
application_sankey = {
  ...
  for (const e of citations_t) {
    ...
    for (const a of cited_algs) {
      // Only sequencing methods (kind='algorithm') with a family — excludes
      // reviews, benchmarks, other downstream apps, etc.
      if (a.kind !== "algorithm" || !a.family) continue
