...
`publication.publication_type` is a string and the SQL column comment is stale: it names only `'preprint'` / `'peer-reviewed'`, but the full vocabulary in use is `'peer-reviewed'` (186), `'preprint'` (71), `'thesis'` (14), `'ML conference'` (9), `'resource'` (3, for field resources that have no manuscript: this catalog's own Zenodo record, a third-party link collection, and a daily literature-briefing Space), `'postprint'` (1) and `'commentary'` (1). Use one of those seven; do not invent an eighth without updating this list, and never leave it empty.
...
Coverage is 234/285. The 51 without one are mostly theses, conference pages and
