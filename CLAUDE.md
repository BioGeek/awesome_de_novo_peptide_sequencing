...
- **Which date.** For a journal with a print issue, use the **issue** date, not
  ...
  issue.
- **Coarser precision.** Month-only sources get `YYYY-MM-01`; 87 rows use day
  `01` and 82 of those are in non-January months, so a first-of-the-month date is
...
Coverage is 249/304. The 55 without one are mostly theses, conference pages and
