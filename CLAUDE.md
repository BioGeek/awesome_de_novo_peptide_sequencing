...
- **Which date.** For a journal with a print issue, use the **issue** date, not
  ...
  issue.
- **Coarser precision.** Month-only sources get `YYYY-MM-01`; 85 rows use day
  `01` and 80 of those are in non-January months, so a first-of-the-month date is
...
Coverage is 248/302. The 54 without one are mostly theses, conference pages and
